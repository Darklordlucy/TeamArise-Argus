...
/* ─── Constants ─────────────────────────────────────────────────── */
const DEFAULT_CENTER = [73.1175, 18.9894]; // [lng, lat] — Panvel
const DEFAULT_ZOOM   = 13.5;
const DEFAULT_PITCH  = 52;
const DEFAULT_BEARING = -20;

const QUICK_ROUTES = [
  { label: "Panvel → Kharghar",  origin: [18.9894, 73.1175], dest: [19.0477, 73.0769] },
  { label: "Belapur → Vashi",    origin: [19.0220, 73.0297], dest: [19.0771, 72.9988] },
  { label: "Panvel → Belapur",   origin: [18.9894, 73.1175], dest: [19.0220, 73.0297] },
];
...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    m.on("load", () => {
      ...
      m.addLayer({
        ...
        paint: { "line-color": C.fast, "line-width": 14, "line-opacity": 0.14, "line-blur": 5 },
      });
      m.addLayer({
        id: "fast-route-line", type: "line", source: "fast-route",
        paint: {
          "line-color": C.fast, "line-width": 4, "line-opacity": 0.78,
          "line-dasharray": [8, 5], "line-cap": "round",
        },
      });
      ...
      m.on("click", "sb-pothole-circles", (e) => {
        ...
        openHazardPopup(e.lngLat, {
          type: "🕳 Pothole (Live)", potholes: 1, crashes: 0,
      ...
      m.on("click", "sb-crash-circles", (e) => {
        openHazardPopup(e.lngLat, {
          type: "💥 Crash Report (Live)", potholes: 0, crashes: 1,
          score: "Reported", color: "#ff1744",
        });
      });
      ...
      m.on("click", "hazard-circles", (e) => {
        const p = e.features[0].properties;
        openHazardPopup(e.lngLat, {
          type: "Hazard Zone", potholes: p.pothole_count, crashes: p.crash_count,
          score: (p.danger_probability * 100).toFixed(1) + "%", color: C.hazard,
        });
      });
      m.on("click", "blackspot-circles", (e) => {
        const p = e.features[0].properties;
        openHazardPopup(e.lngLat, {
          type: "⚠ Accident Blackspot", potholes: p.pothole_count, crashes: p.crash_count,
          score: (p.danger_probability * 100).toFixed(1) + "%", color: C.black,
        });
      });
  ...
  function openHazardPopup(lngLat, data) {
    ...
    popupRef.current = new mapboxgl.Popup({
      closeButton: true, closeOnClick: true, maxWidth: "260px", className: "arg-popup",
    })
      .setLngLat(lngLat)
      .setHTML(`
  ...
  function paintRoutes(data) {
    ...
    m.getSource("hazards").setData({
      type: "FeatureCollection",
      features: points.filter(f => !f.properties.blackspot_present),
    });
    m.getSource("blackspots").setData({
      type: "FeatureCollection",
      features: points.filter(f => f.properties.blackspot_present),
    });
  }
    ...
    <div className="arg-map-page">
      ...
      <div className="arg-planner-card">
        ...
        <div className="arg-quick-routes">
          {QUICK_ROUTES.map(qr => (
            <button key={qr.label} className="arg-quick-btn" onClick={() => applyQuickRoute(qr)}>
              {qr.label}
            </button>
          ))}
        </div>
      ...
      {loading && (
        <div className="arg-loading-overlay">
          ...
          <p className="arg-loading-sub">Analysing road segments with live conditions</p>
        </div>
      )}
